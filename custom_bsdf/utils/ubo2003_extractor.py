...
class BtfFromZip:
    ...
    @staticmethod
    def filename_to_angles(filename: str) -> AnglesTuple:
    ...
    def filename_to_image(self, filename: str) -> np.ndarray:
        ...
        filepaths = [t for t in self.filepath_set if filename in t]
    ...
    def filepath_to_X_y(self, filepath: str) -> Tuple[np.ndarray, AnglesTuple]:
        """`filepath`にあるファイルの実体(*)と角度情報(**)を返す

        (*) ndarray形式
        (**) それぞれがintの(tl, pl, tv, pv)のタプル
        """
        img = Image.open(self.z.open(filepath))
        X = np.array(img)
